# ============================================================================
# 09_BRAZIL TEST — Does BR–US behave like DE–FR (timing) or US–JP (distance)?
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# pair_id format assumed "XX_YY" (e.g. "BR_US"). If your format differs
# (e.g. "BR-US" or "US_BR"), adjust sep below — the matching is order-proof.
...
print(desc_tab)
# READ: if BR_US stress_jump ~ DE_FR jump  -> supports timing hypothesis
#       if BR_US stress_jump ~ US_JP jump  -> supports distance/integration story
...
jumps <- broom::tidy(m_pairjump) |>
  ...
  mutate(group = case_when(
    pair_id %in% brazil_pairs   ~ "Brazil (high overlap, high distance)",
    pair_id %in% timing_bench   ~ "Europe (high overlap, low distance)",
    pair_id %in% distance_bench ~ "Cross-session (zero overlap)",
    TRUE                        ~ "Other pairs"))

# ----------------------------------------------------------------------------
# 3. The decisive figure — stress jump vs overlap, Brazil highlighted
# ----------------------------------------------------------------------------
fig_brazil <- ggplot(jumps, aes(overlap_share, stress_jump_fe)) +
  ...
  labs(x = "Trading-hour overlap (overlap_share)",
       ...
       colour = NULL,
       title = "Is the stress jump driven by timing? Brazil as the decisive case",
       subtitle = " ") +
...
# ----------------------------------------------------------------------------
# 4. Formal decoupled-pairs estimate — does beta survive on Brazil pairs?
# ----------------------------------------------------------------------------
# Interaction: does the overlap_stress effect differ for Brazil pairs?
monthly_panel <- monthly_panel |>
  mutate(br_pair = as.integer(pair_id %in% brazil_pairs))

m_decoupled <- feols(corr_ij ~ overlap_stress + overlap_stress:br_pair |
                       pair_id + month,
                     data    = monthly_panel,
                     cluster = ~pair_id)
...
#       overlap_stress:br_pair    = Brazil differential
#       If the differential is ~0 and insignificant, Brazil behaves exactly
...
#       Brazil -> caution: pooled beta may partly reflect integration.

# ----------------------------------------------------------------------------
# 5. (Optional) leave-Brazil-out — does the pooled beta depend on Brazil?
# ----------------------------------------------------------------------------
m_noBR <- feols(corr_ij ~ overlap_stress | pair_id + month,
                data    = monthly_panel |> filter(!grepl("BR", pair_id)),
                cluster = ~pair_id)
...
# READ: if beta is stable, Brazil is consistent with — but not solely
